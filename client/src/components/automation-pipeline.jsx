...
function PipelineNode({ position, isActive, step }) {
  return (
    <div
...
function PipelineToken({ isPlaying, speed, onPositionChange }) {
  ...
  return (
    <div
      ...
      style={{
        left: `${position}%`,
        boxShadow:
          "0 0 0 2px #0f172a inset, 0 0 16px #22d3ee, 0 0 32px #a78bfa"
      }}
    />
  );
}

function PipelineTrack({ speed, isPlaying }) {
  return (
    <div className="relative h-36">
      ...
      >
        <div
          ...
          style={{
            background:
              "repeating-linear-gradient(90deg, #22d3ee 0 12px, transparent 12px 24px)",
...
export default function AutomationPipeline() {
  ...
  const handleTokenPositionChange = (tokenPosition) => {
  ...
  return (
    <section
      id="pipeline"
      className="py-24 bg-gradient-to-b from-slate-900/50 to-slate-800/50"
    >
      <style>{`
        @keyframes flow {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-xl text-slate-300 max-w-3xl mx-auto">
            Watch how our automation flows seamlessly connect your business
          ...
          {pipelineSteps.map((step, index) => (
            <div
              ...
              className={`glass-card p-4 rounded-xl text-center transition-all duration-300 ${
                currentStep === index
                  ? "ring-2 ring-cyan-400 bg-slate-700/60"
